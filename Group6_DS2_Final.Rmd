...
I believe that the `study` variable is more of a geographical indicator to distinguish different study sites, and it will not be critical in predicting recovery time. Therefore, I will remove  the `study` variable. 
...
## Understand the relationship with continuous predictors and the binary outcome
```{r warning=FALSE, message=FALSE}
theme1 <- trellis.par.get()
theme1$plot.symbol$col <- rgb(.2, .4, .2, .5)
theme1$plot.symbol$pch <- 16
...
# plotting continuous predictors 
featurePlot(x = model.matrix(brecovery_time ~ ., secondary)[trainRows_sec,c("age", "SBP", "LDL", "height", "weight", "bmi")],
...
## Model 7: Support vector classifier (linear kernel)
```{r warning=FALSE, message=FALSE}
set.seed(2460)

model.svml <- train(brecovery_time ~ . , 
                    data = secondary[trainRows_sec,],
                    method = "svmLinear2",
                    tuneGrid = data.frame(cost = exp(seq(-50, 5,len = 100))),
                    trControl = ctrl1,
                    tuneLength = 10000)
...
## Model 8: Support vector machine (radial kernel)
...
model.svmr <- train(brecovery_time ~ . , secondary,
                     subset = trainRows_sec,
